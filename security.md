...
- **LLM-facilitated static analysis**
  ...
  - From SFT to RL: Demystifying the Post-Training Pipeline for LLM-based Vulnerability Detection [[arxiv'26/02](https://arxiv.org/abs/2602.14012v1)]
    -  tricks
      - filterout too hard and too easy, then curriculum learning + pairwise scheduling (each batch has both data before and after patch) 
    -  Multi-granularity Reward Systems
      -  Detection-based Reward (whether there is a vuln)
      -  Prediction-based Reward (cwe)
      -  Reasoning-based Reward (root cause, llm as a judge)
      -  Specification-based Reward
        - for each sample, let an llm make a rubric based on the ground truth, including
          - Prediction Accuracy (e.g., Correct identification of an integer overflow)
...
- **PoC generation**
  ...
  - Locus: Agentic Predicate Synthesis for Directed Fuzzing [[arxiv'25/08](https://arxiv.org/pdf/2508.21302)]
  - FirmAgent: Leveraging Fuzzing to Assist LLM Agents with IoT Firmware Vulnerability Discovery [[arxiv'26/02](https://netsec.ccert.edu.cn/files/papers/ndss26-firmagent.pdf)]
    ...
    - Procedure:
      ...
      - Use fuzzing with dynamic taint analysis to find sources.
        ...
        - Expanded qemu so that they can find the first instruction that reads the tainted input, and within a function that can reach a sink function, mark that as the source function.
        - update the cfg with indirect calls found by fuzzing
      - Use llm to verify each path from source and sink and generate PoC （actually not agent)
        - Taint agent: give llm each decompiled function along the path and ask llm whether there is a vuln
        - Poc agent: give llm decompiled functions, constraints generated by taint analysis and fuzzing seed, ask llm to generate a PoC.
        - Didn't mention how to validate the PoC.
  
  - Execution-State-Aware LLM Reasoning for Automated Proof-of-Vulnerability Generation [[arxiv'26/02](https://arxiv.org/abs/2602.13574)]
    - Setting: given stack trace
    - Four agents with bash, file ops，grep
      - Vuln analysis
        - Find funcs in crash trace, find command to run bin, root cause analysis
      - Code Instrumentation Agent
        - build a binary with coverage
      - Path Exploration Agent
        - loop N time, each time agent generates a testcase that intend to cover last 3 funcs in the stack trace, then inspect the cov, if covered last 3 funcs, put the testcase in a queue
      - Crash Triggering Agent
        - loop generate - execution, has policy injected based on vuln type.
...
### 🛠️ Techniques 
- VulnResolver: A Hybrid Agent Framework for LLM-Based Automated Vulnerability Issue Resolution [[arxiv'26/01](https://arxiv.org/abs/2601.13933)]
  - Tools: search, symbol search(find def, find ref), python intepreter, edit, run_poc
  - Context Pre-Collection Agent
    - find context, uses search and symbol search(find def, find ref)
  - Safety Property Analysis Agent
    - reason about the root cause and violated safety property, insert SAFETY_PROPERTY_ASSERT and recompile and observe.
  - Patch workflow (human based, not an agent)
    - localization(file, code element) -> gen N patches -> filter out patches failed poc test -> majority voting
